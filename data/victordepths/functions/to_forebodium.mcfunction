...
#title @s title {"text":"Out of Range"}
#title @s subtitle {"text":"Move toward sea level or spawn"}
...
execute in victordepths:the_forebodium positioned ~ 63 ~ if loaded ~ 63 ~ if entity @e[tag=vcd_base, distance=..33, predicate=victordepths:in_forebodium] run scoreboard players set @s vcd_portal_ready 1
